...
import {
  ...
  deletePopKindByIds,
  getPopKindList,
} from "@/api/pop/popKind.ts";
import type { CommonPageData } from "@/types";
import AddDialog from "./components/add-pop-kind.vue";
...
// 获取列表
const getAllPopKindList = async () => {
  try {
    gridOptions.value.loading = true;
    const res = await getPopKindList(pageData.value);
...
const handleDelete = async (record: Record<string, any>) => {
  const res = await deletePopKind({ id: record.id });
  if (res.data.code === 2000) getAllPopKindList();
};

// 批量删除
const handleDeletes = async (ids: number[]) => {
  const res = await deletePopKindByIds({ ids: ids });
  if (res.data.code === 2000) getAllPopKindList();
};
