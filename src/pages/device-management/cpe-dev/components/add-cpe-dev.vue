...
import { addCpeDev, updateCpeDev } from "@/api/cpe/cpeDev.ts";
...
const clickConfirm = async () => {
  try {
    ...
    if (formData.value.id) {
      ...
      result = await updateCpeDev(submitData);
    } else {
      delete submitData.id;
      result = await addCpeDev(submitData);
    }
      ...
      <template #formSlot_1>
        <MTableRadio
          ...
          :parameters="{
            ...
            name: 'ip',
            id: 'ipId',
            path: '../../pages/initialize/uninitialized-cpe-dev/index.vue',
          }"
        ></MTableRadio>
      </template>
    </CommonForm>
