...
import {
  getAdminAddressPoolList,
  deleteAdminAddressPool,
  deleteAdminAddressPoolByIds,
...
onMounted(() => {
  getAllAdminAddressPoolList();
});

// 获取列表
const getAllAdminAddressPoolList = async () => {
  try {
    gridOptions.value.loading = true;
    const res = await getAdminAddressPoolList(pageData.value);
...
const handleDelete = async (record: Record<string, any>) => {
  const res = await deleteAdminAddressPool({ id: record.id });
  if (res.data.code === 2000) getAllAdminAddressPoolList();
};

// 批量删除
const handleDeletes = async (ids: number[]) => {
  const res = await deleteAdminAddressPoolByIds({ ids: ids });
  if (res.data.code === 2000) getAllAdminAddressPoolList();
};
...
<template>
  <page-container :styleConfig="props.styleConfig">
    ...
    <AddDialog
      ...
      @refreshList="getAllAdminAddressPoolList()"
    ></AddDialog>
  </page-container>
</template>
